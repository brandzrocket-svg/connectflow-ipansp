...
export default function AreaPage() {
  ...
  const { eventos, refresh: refreshEventos } = useEventos(year, month);
  const { escalas, saveEscala, removeEscala, refresh: refreshEscalas } = useEscalas(area?.id);
  const [showForm, setShowForm] = useState(false);
  const [escalaEditando, setEscalaEditando] = useState<Escala | undefined>(undefined);
  const [confirmDelete, setConfirmDelete] = useState<string | null>(null);
  const [todosEventos, setTodosEventos] = useState(getEventos());

  useEffect(() => {
    refreshEventos();
    setTodosEventos(getEventos());
  ...
  function confirmDeleteEscala() {
    if (confirmDelete) {
      removeEscala(confirmDelete);
    ...
    <div className="min-h-screen" style={{ backgroundColor: '#000000' }}>
      ...
      {showForm && (
        <EscalaForm
          ...
          onSave={(data) => { saveEscala(data); refreshEscalas(); }}
